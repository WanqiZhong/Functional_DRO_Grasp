...
from data_utils.CombineRetargetDatasetMultiContact import create_dataloader
from model.network import create_network_larger_transformer_clip_add_dgcnn
from model.module import TrainingModule


@hydra.main(version_base="1.2", config_path="configs", config_name="train_clip_512_add_dgcnn_contact")
def main(cfg):
    print("******************************** [Config] ********************************")
    ...
    pl.LightningModule.strict_loading = False
    trainer = pl.Trainer(
        logger=logger,
        accelerator='gpu',
    ...
    dataloader = create_dataloader(cfg.dataset, mode='train', fixed_initial_q=False)

    network = create_network_larger_transformer_clip_add_dgcnn(cfg.model, mode='train')
    ...
    # Set trainer to not strict load
