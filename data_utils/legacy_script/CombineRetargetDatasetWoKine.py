...
class CombineDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            complex_openai_embedding_batch = []
            ...
            for idx, robot_name in enumerate(robot_names):
                ...
                if robot_name == 'mano' or robot_name == 'retarget_shadowhand':
                    # metadata_robot = [m for m in self.oakink_metadata if m[6] == robot_name]
                    if self.fix_sample:
                        hand_pose, hand_shape, tsl, target_q, intent, object_name, _, object_id, _, hand_verts, complex_sentence,  complex_embedding, complex_openai_embedding = metadata_robot[s_idx]
                        s_idx += 1
                    else:
                        hand_pose, hand_shape, tsl, target_q, intent, object_name, _, object_id, _, hand_verts, complex_sentence,  complex_embedding, complex_openai_embedding = random.choice(metadata_robot)
                    ...
                    _, _, _, _, _, cross_object_name, _, cross_object_id, _, cross_hand_verts, _, _ = cross_data[:12]

                    complex_language_embedding_batch.append(complex_embedding)
                    complex_language_sentence_batch.append(complex_sentence)
                    complex_openai_embedding_batch.append(complex_openai_embedding)
                else:
                    # metadata_robot = [(m[0], m[1]) for m in self.cmap_metadata if m[2] == robot_name]
                    # >>> Debug Only >>>
                ...
                initial_q_batch.append(initial_q)
                # robot_pc_initial = hand.get_transformed_links_pc(initial_q, only_palm=self.only_palm)[:, :3]
                # robot_pc_initial_batch.append(robot_pc_initial)
            ...
            if len(complex_language_embedding_batch) == B:
                ...
                complex_openai_embedding_batch = torch.stack(complex_openai_embedding_batch)
            else:
            ...
            return {
                ...
                # 'robot_pc_nofix_initial': robot_pc_nofix_initial_batch,
                ...
                'nofix_initial_q': nofix_initial_q_batch,
    ...
    def _get_object_pc(self, object_name, object_id, robot_name):
        if self.object_pc_type == 'fixed':
            name = object_name.split('+')
