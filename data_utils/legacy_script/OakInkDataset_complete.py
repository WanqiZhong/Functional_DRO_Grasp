...
class OakInkDataset(Dataset):
    def __init__(
        ...
        if not self.is_train:
            self.combination = [(m[5], m[6], m[7]) for m in self.metadata if m[6] in self.robot_names and m[5] in self.object_names]
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            for idx in range(self.batch_size):  
                ...
                metadata_robot = [(m[3], m[5], m[7]) for m in self.metadata if m[6] == robot_name]

                target_q, object_name, object_id = random.choice(metadata_robot)
            ...
            object_name, robot_name, object_id = self.combination[index]
