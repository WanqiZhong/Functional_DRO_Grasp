# Support error handling for obj2urdf
...
def main() -> None:
    ...
    timeout_seconds = 600  # 超时时间：10分钟

    with multiprocessing.Pool(args.num_processes) as pool:
        results = []
        for params in pool_params:
            result = pool.apply_async(process_obj_onelink, params)
            results.append((params[0], result)) 

        for obj_file, result in results:
            try:
                result.get(timeout=timeout_seconds)  # 等待任务完成，超时则异常
            except multiprocessing.TimeoutError:
                print(f"超时退出: {obj_file}")
            except Exception as e:
                print(f"处理 {obj_file} 失败: {e}")
                traceback.print_exc()
        
        pool.terminate()  # 终止所有任务，防止进程池卡死
