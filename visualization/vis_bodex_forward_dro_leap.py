...
from utils.rotation_format_utils import leaphand_from_mujoco_to_bodex, leaphand_from_bodex_to_oakink, leaphand_from_bodex_to_mujoco
import pickle 
import warnings
warnings.filterwarnings("ignore")

robot_name = "leaphand"
BENCH_DATA_PATH = "/data/zwq/code/DexGraspBench/output/oakink_teapot_step020_max_leap_bodex/succgrasp/teapot/o13104/1.npy"
BODEX_DATA_PATH = "/data/zwq/code/BODex/src/curobo/content/assets/output/sim_leap/fc/oakink_teapot_step020_max/graspdata/o13104/scale180_pose000_2grasp.npy"
# DRO_MUJOCO_DATA_PATH = "/data/zwq/code/DRO_Grasp/data/OakInkDataset/teapot_oakink_dataset_standard_all_retarget_to_leaphand_mujoco.pt"
DRO_DATA_PATH = "/data/zwq/code/DRO_Grasp/data/OakInkDataset/teapot_oakink_dataset_standard_all_retarget_to_leaphand_bodex_valid.pt"
DRO_OBJECT_PATH = "/data/zwq/code/DRO_Grasp/data/data_urdf/object/oakink/teapot/o13104.obj"
HAND_VSI_OBJ_PATH = "/data/zwq/code/DexGraspBench/output/oakink_teapot_step020_max_leap_bodex/vis_obj/teapot/o13104/1_grasp_0.obj"
OBJECT_VIS_OBJ_PATH = "/data/zwq/code/DexGraspBench/output/oakink_teapot_step020_max_leap_bodex/vis_obj/teapot/o13104/1_obj.obj"
...
def main():
    ...
    try:
        ...
        print(f"Origin bench q: {bench_q}")
        bench_q = leaphand_from_mujoco_to_bodex(bench_q)
        print(f"Bench_q: {bench_q}")
        bench_q = leaphand_from_bodex_to_oakink(bench_q)
    ...
    try:
        ...
        bodex_q = bodex_q[0][1]
        print(f"BODex q: {bodex_q}")
        bodex_q = leaphand_from_bodex_to_oakink(bodex_q)
    ...
    try:
        ...
        print(f"Loaded DRO dataset with keys: {list(mujoco_dataset_data.keys())}")
        mujoco_dataset_q = mujoco_dataset_data['metadata'][0][3]
        mujoco_dataset_q = leaphand_from_bodex_to_oakink(mujoco_dataset_q)
        ...
        print(f"Error loading DRO dataset: {e}")
