...
def main(cfg):
    ...
    def on_update(robot_id, intent_id, object_id, grasp_id):
        ...
        hand_pose, hand_shape, hand_tsl, q, _, object_name, _, object_id, _, hand_verts_pc, _, _ = data

        indices = torch.randperm(65536)[:512]
        object_pc = object_pcs[object_id][indices]
        object_pc += torch.randn(object_pc.shape) * 0.002

        print(f"Input {grasp_id} recognized object: {object_name} with intent {intent}")

        # if object_name in train_objects:
            # print("Using original object")
        similar_entries = data
        # else:
            # similar_entries = find_similar_objects(topk_similar_names, object_name, category_intent_to_entries, intent)
            # if not similar_entries:
                # print(f"No matching object found for {object_name} with intent {intent}")
                # return

        _, _, _, target_q, _, _, _, _, _, origin_hand_verts, _, _ = similar_entries
        initial_q = hand.get_initial_q(target_q).unsqueeze(0).to(device)
        ...
        object_vertices, object_faces = get_object_vertices(object_name, object_id)
        ...
        initial_robot_trimesh = hand.get_trimesh_q(initial_q)["visual"]
        ...
        server.scene.add_mesh_simple(
            'initial_robot',
            initial_robot_trimesh.vertices,
            initial_robot_trimesh.faces,
            color=(102, 192, 255),
            opacity=0.8
        )
