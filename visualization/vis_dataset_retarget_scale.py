...
def on_update(robot_idx, object_idx, grasp_idx):
    ...
    if "oakink" in object_name:
        ...
        if len(grasp_item) > 10:
            scale_factor = grasp_item[10].item() 
        else:
            scale_factor = 1.0
    ...
    origin_dimension = max(object_trimesh_original.bounding_box.extents)
    scaled_dimension = max(object_trimesh_scaled.bounding_box.extents)
    ...
    if mano_compatibility:
        ...
        dimension_info_text.value = f"O: {origin_dimension:.3f} | S: {scaled_dimension:.3f} | F: {scale_factor:.3f}"
...
dimension_info_text = server.gui.add_text("尺寸信息", "")
