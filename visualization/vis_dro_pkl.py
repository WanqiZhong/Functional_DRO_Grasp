...
CONVERTED_DATASET_PATH = '/data/zwq/code/DRO_Grasp/output/model_origin/res_diffuser_dro_predict_q.pkl'
...
def load_object_mesh(object_key, object_id):
    ...
    obj_mesh_path = list(
        ...
        glob.glob(os.path.join(ROOT_DIR, f'data/data_urdf/object/{name[0]}/{name[1]}/{name[1]}.stl')))
...
converted_data = pickle.load(open(CONVERTED_DATASET_PATH, 'rb'))
converted_metadata = converted_data['results']
...
def on_update(item_idx, bodex_idx):
    ...
    converted_robot_value = converted_item['predict_q']
    object_key = converted_item['object_name'][0]  # e.g., 'oakink+teapot'
    object_id = converted_item['object_id'][0]  # e.g., 'C90001'
    ...
    #     'function',
    #     f'{converted_item['complex_language']}',
    #     wxyz=(1, 0, 0, 0),
    #     position=(1, 1, 1)
    # )
