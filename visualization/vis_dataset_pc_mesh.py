...
point_cloud_dataset = torch.load(os.path.join(ROOT_DIR, 'data', 'OakInkDataset', 'oakink_object_pcs.pt'), weights_only=True)
metadata = torch.load(dataset_path, map_location=torch.device('cpu'), weights_only=True)['metadata']
...
def on_update(robot_idx, object_idx, grasp_idx, num_points=512):
    ...
    object_info.content = f"Functional Sentence: {complex_sentence}\n Object ID: {object_id}\n Size: x={object_size[0]:.2f}, y={object_size[1]:.2f}, z={object_size[2]:.2f}\n"
