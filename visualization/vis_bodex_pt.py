...
def on_update(item_idx, bodex_idx):
    ...
    converted_item = converted_metadata[item_idx][0]
    bodex_item = converted_metadata[item_idx][1][:, 1, :][bodex_idx]
    ...
    bodex_robot_value = torch.cat([bodex_robot_value[:3], euler, torch.tensor([0.0, 0.0]), bodex_robot_value[12:], bodex_robot_value[7:12]], axis=-1).float()
    ...
    bodex_hand_mesh = hand.get_trimesh_q(bodex_robot_value)["visual"]
    
    # Add original hand mesh (left side)
    server.scene.add_mesh_simple(
        'bodex_hand',
        bodex_hand_mesh.vertices,  
        bodex_hand_mesh.faces,
