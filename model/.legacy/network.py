...
from model.encoder import create_encoder_network
...
class Network(nn.Module):
    ...
    def forward(self, robot_pc, object_pc, target_pc=None, intent=None, 
        ...
        # CVAE encoder
        if self.cfg.use_language:
            z = self.language_fc(language_emb)
            mu, logvar = None, None
        ...
        z = z.unsqueeze(dim=1).repeat(1, robot_embedding_tf.shape[1], 1)  # (B, N, latent_dim + intent_dim)
...
class NetworkLarge(nn.Module):
    def __init__(self, cfg, mode):
        ...
        self.transformer_robot = Transformer(emb_dim=cfg.emb_dim+cfg.latent_dim)
        self.transformer_object = Transformer(emb_dim=cfg.emb_dim+cfg.latent_dim)
        ...
        if cfg.use_intent:
            self.kernel = MLPKernelLarge(cfg.emb_dim + cfg.latent_dim + cfg.intent_dim)
        else:
            self.kernel = MLPKernelLarge(cfg.emb_dim + cfg.latent_dim)
...
def create_network_large(cfg, mode):
    network = NetworkLarge(
        cfg=cfg,
        mode=mode
    )
    return network

def create_three_dof_network(cfg, mode):
    network = ThreeDoFNetwork(
        cfg=cfg,
        mode=mode
    )
    return network
...
class Network_larger_transformer(nn.Module):
    def __init__(self, cfg, mode):
        super(Network_larger_transformer, self).__init__()
        self.cfg = cfg
        self.mode = mode

        self.encoder_robot = create_encoder_network(emb_dim=cfg.emb_dim, pretrain=cfg.pretrain)
        self.encoder_object = create_openad_pn_encoder_network(pretrain=cfg.obj_pretrain)
        ...
        if cfg.use_language:
            self.language_fc = nn.Linear(768, cfg.latent_dim)
            nn.init.xavier_uniform_(self.language_fc.weight)
            if self.language_fc.bias is not None:
                nn.init.zeros_(self.language_fc.bias)
    ...
    def forward(self, robot_pc, object_pc, target_pc=None, intent=None, 
        ...
        # point cloud encoder
        robot_embedding = self.encoder_robot(robot_pc)
        object_embedding, _ = self.encoder_object(object_pc.permute(0, 2, 1))
...
def create_network_larger_transformer(cfg, mode):
    network = Network_larger_transformer(
        cfg=cfg,
        mode=mode
    )
    return network
...
class Network_larger_transformer_openai(nn.Module):
    def __init__(self, cfg, mode):
        super(Network_larger_transformer_openai, self).__init__()
        self.cfg = cfg
        self.mode = mode
...
def create_network_larger_transformer_openai(cfg, mode):
    network = Network_larger_transformer_openai(
        cfg=cfg,
        mode=mode
    )
    return network
...
class Network_larger_transformer_openai_dgcnn(nn.Module):
    def __init__(self, cfg, mode):
        super(Network_larger_transformer_openai_dgcnn, self).__init__()
        self.cfg = cfg
        self.mode = mode
...
def create_network_larger_transformer_openai_dgcnn(cfg, mode):
    network = Network_larger_transformer_openai_dgcnn(
        cfg=cfg,
        mode=mode
    )
    return network
...
class Network_larger_transformer_clip_dgcnn(nn.Module):
    def __init__(self, cfg, mode):
        super(Network_larger_transformer_clip_dgcnn, self).__init__()
        self.cfg = cfg
        self.mode = mode
...
def create_network_larger_transformer_clip_dgcnn(cfg, mode):
    network = Network_larger_transformer_clip_dgcnn(
        cfg=cfg,
        mode=mode
    )
    return network
...
class Network_larger_transformer_clip_dgcnn_add(nn.Module):
    def __init__(self, cfg, mode):
        super(Network_larger_transformer_clip_dgcnn_add, self).__init__()
        self.cfg = cfg
        self.mode = mode
        ...
        self.transformer_robot = Transformer(emb_dim=cfg.emb_dim, ff_dims=2*(cfg.emb_dim), n_blocks=2)
        self.transformer_object = Transformer(emb_dim=cfg.emb_dim, ff_dims=2*(cfg.emb_dim), n_blocks=2)
...
def create_network_larger_transformer_clip_dgcnn_add(cfg, mode):
    network = Network_larger_transformer_clip_dgcnn_add(
        cfg=cfg,
        mode=mode
    )
    return network

class Network_mix_transformer_openai(nn.Module):
    def __init__(self, cfg, mode):
        super(Network_mix_transformer_openai, self).__init__()
        self.cfg = cfg
        self.mode = mode
        ...
        self.transformer_robot = Transformer(emb_dim=cfg.emb_dim+cfg.latent_dim, ff_dims=2*(cfg.emb_dim+cfg.latent_dim), n_blocks=4)
        self.transformer_object = Transformer(emb_dim=cfg.emb_dim+cfg.latent_dim, ff_dims=2*(cfg.emb_dim+cfg.latent_dim), n_blocks=4)
    ...
    def forward(self, robot_pc, object_pc, target_pc=None, intent=None, 
        ...
        # point cloud encoder
        robot_embedding = self.encoder_robot(robot_pc)
        object_embedding, object_pn_embedding = self.encoder_object(object_pc.permute(0, 2, 1))
...
def create_network_mix_transformer_openai(cfg, mode):
    network = Network_mix_transformer_openai(
        cfg=cfg,
        mode=mode
    )
    return network
...
class Network_larger_transformer_openai_openad_dgcnn(nn.Module):
    def __init__(self, cfg, mode):
        super(Network_larger_transformer_openai_openad_dgcnn, self).__init__()
        self.cfg = cfg
        self.mode = mode

        self.encoder_robot = create_encoder_network(emb_dim=cfg.emb_dim, pretrain=cfg.pretrain)
        self.encoder_object = create_openad_dgcnn_encoder_network(pretrain=cfg.obj_pretrain)
...
def create_network_larger_transformer_openai_openad_dgcnn(cfg, mode):
    network = Network_larger_transformer_openai_openad_dgcnn(
        cfg=cfg,
        mode=mode
    )
    return network