...
from model.network import create_network_larger_transformer_openai
from model.module import TrainingModule


@hydra.main(version_base="1.2", config_path="configs", config_name="train_mix_retarget_language_larger_transformer_openai_256_fix_initial")
def main(cfg):
    print("******************************** [Config] ********************************")
    ...
    dataloader = create_dataloader(cfg.dataset, is_train=True, fixed_initial_q=True)

    network = create_network_larger_transformer_openai(cfg.model, mode='train')
