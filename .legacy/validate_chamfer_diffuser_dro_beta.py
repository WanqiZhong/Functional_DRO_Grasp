...
def _get_object_pc(object_names, object_ids, robot_names, batch_size):
    ...
    for i in range(batch_size):
        ...
        ) if (robot_names[i] == 'mano' or robot_names[i] == 'retarget_shadowhand') else os.path.join(
...
def main(cfg):
    ...
    for batch_idx, batch_data in enumerate(tqdm(results)):
        ...
        for k in range(k_sample):
            ...
            for sample_idx in range(batch_size):
                chamfer_value = (d1[sample_idx].sum() + d2[sample_idx].sum()).item()
                chamfer_values.append(chamfer_value)
                ...
                sample_data['chamfer_value'] = chamfer_values[sample_idx]
